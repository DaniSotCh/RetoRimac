...
const CollapseForm: FunctionComponent<CollapseForm> = (props) => {
  ...
  const ButtonPlan = (addPlan: any, removePlan: any, step: number, isAdd: boolean) => {
    const text = 'He salido de casa a las cuatro menos cinco para ir a la academia de ingles de mi pueblo (Sant Cugat, al lado de Barcelona) con mi bici, na llego a la academia que está en el centro del pueblo en una plaza medio-grande y dejo donde siempre la bici atada con una pitón a un sitio de esos de poner las bicis y mucho más';
    return (
      <div>
        {isAdd && <div><Button type="link" className='link-text' htmlType="button" onClick={() => addPlan(step)} icon={<PlusCircleOutlined />}>Agregar</Button></div>}
        {!isAdd && <div>
          <Button type="link" className='link-text' htmlType="button" onClick={() => removePlan(step)} icon={<MinusCircleOutlined />}>Quitar</Button>
  ...
  return (
    <Collapse defaultActiveKey={['1']} ghost expandIconPosition='right'>
      <Panel className='subtitle06' header="Llanta robada" key="1"
        extra={
          <img alt='' width='48' src='https://www.rimac.com/content/dam/rimac/publica/pagina-producto/vehicular-flexible/simulador/protege_vehiculo/icon_roboparcial.svg'></img>
        }>
        <div className='m-lr'>
          {ButtonPlan(addPlan, removePlan, 1, isAdd.step1)}
        </div>
      </Panel>
      <Panel className='subtitle06' header="Choque y/o pasarte la luz roja" key="2"
        extra={
          <img alt='' width='48' src='https://www.rimac.com/content/dam/rimac/publica/pagina-producto/vehicular-flexible/simulador/protege_vehiculo/icon_perdidaparcial.svg'></img>
        }>
        <div className='m-lr'>
          {ButtonPlan(addPlan, removePlan, 2, isAdd.step2)}
        </div>
      </Panel>
      <Panel className='subtitle06' header="Atropello en la vía Evitamiento" key="3"
        extra={
          <img alt='' width='48' src='https://www.rimac.com/content/dam/rimac/publica/pagina-producto/vehicular-flexible/simulador/protege_vehiculo/icon_perdidatotal.svg'></img>
        }>
        <div className='m-lr'>
